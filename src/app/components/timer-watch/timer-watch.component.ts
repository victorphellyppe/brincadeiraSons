...
export class TimerWatchComponent implements OnInit, AfterViewInit {
  ...
  setStatusTouch(event: TouchEvent, status: number) {
    if (status === 1) event.stopPropagation();
    else if (status === 2)
      this.mouseClick = {
  ...
  onTouchMove(event: TouchEvent) {
    this.mouse = { x: event.touches[0].pageX, y: event.touches[0].pageY };
